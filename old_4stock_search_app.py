...
if uploaded_file:
    ...
    # Filter by condition
    conditions = st.multiselect("Choisissez une condition", options=filtered_data_by_sub_category["Condition"].unique().tolist(), default=filtered_data_by_sub_category["Condition"].unique().tolist())
    ...
    # Filter results
    final_filtered_data = filtered_data_by_sub_category[filtered_data_by_sub_category["Producent Kode"].isin(brands) & (filtered_data_by_sub_category["Kalkuleret Kostpris (RV)"] >= min_price) & (filtered_data_by_sub_category["Kalkuleret Kostpris (RV)"] <= max_price) & (filtered_data_by_sub_category["Condition"].isin(conditions))]
    ...
    for index, row in final_filtered_data.iterrows():
        ...
        st.write(f'**Lien:** [Cliquez ici]({row["Web URL"]})')  # Directly use the provided link without any modification
        st.write("---")
